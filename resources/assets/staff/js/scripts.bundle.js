        ...
        setActiveMenu: function() {
            $(function($) {
                ...
                $('li a').each(function() {
                    ...
                    if(!isNaN(new_url.substring(new_url.lastIndexOf('/')+1))){
                        new_url = new_url.substring(0, new_url.lastIndexOf('/'));
                    }
                    if (this.href === url || this.href == new_url) {
                        var parent = KTUtil.parents(this, '.menu-item');
                        Plugin.setActiveItem(parent[0]);
                    }
                });
            });
        },
        ...
        */
        ...
        setActiveItem: function(item) {
            ...
            for (var i = 0, len = parents.length; i < len; i++) {
                var child_item = KTUtil.child(parents[i], '.menu-toggle');
                if(child_item){
                    var li = child_item.closest('.menu-item');
                    var submenu = KTUtil.child(li, '.menu-submenu, .menu-inner');
                    var speed = the.options.accordion.slideSpeed;

                    KTUtil.slideDown(submenu, speed, function() {
                        Plugin.scrollToItem(child_item);
                        Plugin.scrollUpdate();
